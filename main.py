from UserData import userData
from StockData import stockData

class PaperInvestApp(userData, stockData):
    def __init__(self):
        super().__init__()
        self.user = self.load_data()
        self.current_user = None

    def run(self):
        print("Welcome to PaperInvest! - Stock Market Simulator")

        while not self.current_user:
            print("\n1. Login\n2. Create Account\n3. Exit")
            choice = input("Choose an option")

            if choice == "1":
                self.current_user = self.login(self.user) #login returns the username so current_user will hold onto it
            elif choice == "2":
                self.current_user = self.create_account(self.user) #create_account returns the username
            elif choice == "3":
                return
            else:
                print("Invalid Option")

        user = self.user[self.current_user] #user[current_user] will contain a username (key)

        while True:
            print(f"\n💰Balance: ${round(user['balance'],2)} | Portfolio Value: ${self.portfolio_value(user)}")
            print("\n1. View Portfolio")
            print("2. Buy Stock")
            print("3. Sell Stock")
            print("4. View Transactions")
            print("5. View Top Gainers")
            print("6. View Top Losers")
            print("7. view Daily Gain/loss")
            print("8. Save & Exit")

            choice = input("Select An Option")

            match choice:
                case "1":
                    self.view_portfolio(user)
                case "2":
                    self.buy_stock(user)
                case "3":
                    self.sell_stock(user)
                case "4":
                    self.view_transactions(user)
                case "5":
                    self.displayTopGainers()
                case "6":
                    self.displayTopLossings()
                case "7":
                    self.show_daily_gain_loss(user)
                case "8":
                    self.save_data(self.user)
                    break
                case _:
                    print("Invalid Option")

        self.save_data(self.user)


if __name__ == "__main__":
    app = PaperInvestApp()
    app.run()
